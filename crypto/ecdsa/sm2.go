...
func ToGmPrivateKey(priv *PrivateKey) *sm2.PrivateKey {
...
func ToGmPublicKey(pub *PublicKey) *sm2.PublicKey {
	return &sm2.PublicKey{sm2.P256Sm2(), pub.X, pub.Y}
}
...
func Sign(rand io.Reader, priv *PrivateKey, hash []byte) (r, s *big.Int, err error) {
	if priv.Curve.Params().Name == "P-256" {
		return sm2.Sm2Sign(ToGmPrivateKey(priv), hash, nil, rand)
	}
...
func Verify(pub *PublicKey, hash []byte, r, s *big.Int) bool {
	if pub.Curve.Params().Name == "P-256" {
		gmPub := ToGmPublicKey(pub)
		if sm2.Sm2Verify(gmPub, hash, nil, r, s) {
			return true
		} else {
			highS := new(big.Int).Sub(pub.Curve.Params().N, s)
			return sm2.Sm2Verify(gmPub, hash, nil, r, highS)
		}
	}
	// See [NSA] 3.4.2
	c := pub.Curve
