...
import (
	...
	"github.com/gxnublockchain/gmsupport/internal/x/net/http/httpguts"
	"github.com/gxnublockchain/gmsupport/internal/x/net/http/httpproxy"
)
...
func (t *Transport) roundTrip(req *Request) (*Response, error) {
	...
	if isHTTP {
		for k, vv := range req.Header {
			if !httpguts.ValidHeaderFieldName(k) {
				req.closeBody()
				return nil, fmt.Errorf("github.com/gxnublockchain/gmsupport/net/http: invalid header field name %q", k)
			}
			for _, v := range vv {
				if !httpguts.ValidHeaderFieldValue(v) {
					req.closeBody()
					return nil, fmt.Errorf("github.com/gxnublockchain/gmsupport/net/http: invalid header field value %q for key %v", v, k)
				}
			}
		}
	}
	...
	if req.Method != "" && !validMethod(req.Method) {
		req.closeBody()
		return nil, fmt.Errorf("github.com/gxnublockchain/gmsupport/net/http: invalid method %q", req.Method)
	}
	if req.URL.Host == "" {
		req.closeBody()
...
var ErrSkipAltProtocol = errors.New("github.com/gxnublockchain/gmsupport/net/http: skip alternate protocol")
...
func (e transportReadFromServerError) Error() string {
	return fmt.Sprintf("github.com/gxnublockchain/gmsupport/net/http: Transport failed to read from server: %v", e.err)
}
...
func (t *Transport) dial(ctx context.Context, network, addr string) (net.Conn, error) {
	...
	if t.Dial != nil {
		...
		if c == nil && err == nil {
			err = errors.New("github.com/gxnublockchain/gmsupport/net/http: Transport.Dial hook returned (nil, nil)")
		}
		return c, err
	}
...
func (w *wantConn) tryDeliver(pc *persistConn, err error) bool {
	...
	w.err = err
	if w.pc == nil && w.err == nil {
		panic("github.com/gxnublockchain/gmsupport/net/http: internal error: misuse of tryDeliver")
	}
	close(w.ready)
	return true
}
...
func (t *Transport) customDialTLS(ctx context.Context, network, addr string) (conn net.Conn, err error) {
	...
	if conn == nil && err == nil {
		err = errors.New("github.com/gxnublockchain/gmsupport/net/http: Transport.DialTLS or DialTLSContext returned (nil, nil)")
	}
	return
}
...
func (t *Transport) decConnsPerHost(key connectMethodKey) {
	...
	if n == 0 {
		...
		panic("github.com/gxnublockchain/gmsupport/net/http: internal error: connCount underflow")
	}
...
func (pc *persistConn) mapRoundTripError(req *transportRequest, startBytesWritten int64, err error) error {
	...
	if pc.isBroken() {
		...
		return fmt.Errorf("github.com/gxnublockchain/gmsupport/net/http: HTTP/1.x transport connection broken: %v", err)
	}
	return err
}
...
func (pc *persistConn) readLoop() {
	...
	for alive {
		...
		if err != nil {
			if pc.readLimit <= 0 {
				err = fmt.Errorf("github.com/gxnublockchain/gmsupport/net/http: server response headers exceeded %d bytes; aborted", pc.maxHeaderResponseSize())
			}
	...
	for {
		...
		if is1xxNonTerminal {
			...
			if num1xx > max1xxResponses {
				return nil, errors.New("github.com/gxnublockchain/gmsupport/net/http: too many 1xx informational responses")
			}
...
var errTimeout error = &httpError{err: "github.com/gxnublockchain/gmsupport/net/http: timeout awaiting response headers", timeout: true}

// errRequestCanceled is set to be identical to the one from h2 to facilitate
// testing.
var errRequestCanceled = http2errRequestCanceled
var errRequestCanceledConn = errors.New("github.com/gxnublockchain/gmsupport/net/http: request canceled while waiting for connection") // TODO: unify?
...
func (tlsHandshakeTimeoutError) Error() string   { return "github.com/gxnublockchain/gmsupport/net/http: TLS handshake timeout" }
